...
import { fetchDeepseekChatUpstream } from '../api/_lib/deepseekUpstream.ts';

/** 本地 `npm run dev` 时模拟 `/api/deepseek-chat` */
export function createDeepseekDevMiddleware(opts: { apiKey: string; apiBase: string }) {
  return async function deepseekDevMiddleware(
    req: IncomingMessage,
    res: ServerResponse,
    next: () => void,
  ): Promise<void> {
    const url = req.url ?? '';
    if (!url.startsWith('/api/deepseek-chat')) {
      next();
      return;
    }
    if (req.method !== 'POST') {
    ...
    if (!opts.apiKey) {
      ...
      res.end(
        JSON.stringify({
          error: '本地请在 .env.local 配置 DEEPSEEK_API_KEY（仅服务端使用，不会打进前端）',
        }),
      );
      return;
    }
    ...
    try {
      ...
      console.error('[vite deepseek dev]', e);
      res.statusCode = 502;
      res.setHeader('Content-Type', 'application/json; charset=utf-8');
    ...
    try {
      ...
      console.error('[vite deepseek dev] stream', e);
      if (!res.headersSent) {
        res.statusCode = 502;
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
